  ...
  return (
    <div className="p-6 max-w-5xl mx-auto h-[calc(100vh-5rem)] flex flex-col space-y-4">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-2 shrink-0">
        <span className="text-[11px] font-semibold text-slate-400 uppercase shrink-0 mr-1 flex items-center gap-1">
          <Zap className="w-3.5 h-3.5 text-brand-400" /> Prompts:
        </span>
        {quickPrompts.map((p, idx) => (
          <button
            key={idx}
            onClick={() => handlePromptClick(p)}
            className="px-3 py-1.5 rounded-xl bg-[#141c2e] hover:bg-brand-600 hover:text-white text-slate-300 text-xs font-medium border border-[#23324d] transition-all shrink-0 whitespace-nowrap"
          >
            {p}
          </button>
        ))}
      ...
      <div className="flex-1 glass-panel rounded-2xl p-5 overflow-y-auto space-y-4 border border-[#1f293d]">
        {assistantMessages.map((msg) => {
          ...
          return (
            ...
            >
              {!isUser && (
                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-brand-500 to-indigo-700 flex items-center justify-center text-white shrink-0 shadow-glow border border-brand-400/30">
              ...
              <div
                className={`max-w-2xl rounded-2xl p-4 space-y-3 ${
                  isUser
                    ...
                    : 'bg-[#131c2e] text-slate-200 border border-[#23324d] rounded-bl-none shadow-sm'
                }`}
              >
                {/* Header */}
                <div className="flex items-center justify-between text-[11px] opacity-70 border-b border-white/10 pb-1.5 mb-1.5">
                  <span className="font-bold">{isUser ? 'You' : 'ControlFlow AI Controller'}</span>
